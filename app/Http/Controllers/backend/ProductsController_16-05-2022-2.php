...
//      dd($products);
    ...
    public function edit($id)
    {
        ...
        // dd($product_images);
        $colors = Colors::pluck('color_name','color_code')->all();
        $filters = Filters::with('filtervalues')->where('visibility',1)->where('product_page_visibility',1)->where('category_id',$products->category_id)->orderBy('category_id')->get();
        foreach ($filters as $filter)
        {
          $filter_name = $filter->filter_name.'->';
          foreach ($filter->filtervalues as $filter_value)
    ...
    public function getfilters(Request $request)
    {
      ...
      $filters = Filters::with('filtervalues')->where('visibility',1)->where('product_page_visibility',1)->where('category_id',$category_id)->orderBy('category_id')->get();
