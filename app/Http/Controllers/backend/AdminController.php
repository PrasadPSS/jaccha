...
use App\Models\frontend\PaymentInfo as FrontendPaymentInfo;
use Auth;
use Carbon\Carbon;
use Hash;
...
class AdminController extends Controller
{
  ...
  //only got todays order
   ...
   ->whereRaw('Date(created_at) = CURDATE()')->count();
    ...
    // $payment_mode = PaymentInfo::where('payment_mode', 'payumoney')->whereDay('created_at', now()->day)->get();
    $pre_paid_orders = PaymentInfo::where('payment_mode', 'payumoney')->whereDate('payment_date', '=', date('Y-m-d'))->sum('amount');
    $cod = PaymentInfo::where('payment_mode', 'cod')->whereDate('payment_date', '=', date('Y-m-d'))->sum('amount');

    $total_prepaid_orders = count(PaymentInfo::where('payment_mode', 'payumoney')->whereDate('payment_date', '=', date('Y-m-d'))->get());
    $total_cod = count(PaymentInfo::where('payment_mode', 'cod')->whereDate('payment_date', '=', date('Y-m-d'))->get());
  return view('backend.admin.dashboard',compact('products','user','order','order_id','records','orderlist','customerlist', 'orderpanding', 'orderproccesing', 'orderdeliverd', 'ordercancel', 'orderreturn', 'totalsales','pre_paid_orders', 'cod' ,'total_prepaid_orders' ,'total_cod'));
  }
