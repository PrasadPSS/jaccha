...
class OrdersController extends Controller
{
    ...
    public function updateprogress(Request $request)
    {
      ...
      {
        // dd($order->invoice_counter_id == null || $order->invoice_counter_id == "");
        // dd(($order->confirmed_stage == 0 || $order->confirmed_stage == null || $order->confirmed_stage == ""));
        if(($order->confirmed_stage == 0 || $order->confirmed_stage == null || $order->confirmed_stage == "") && isset($request->confirmed_stage))
        {
          ...
          if($order->invoice_counter_id == null || $order->invoice_counter_id == "")
          {
          ...
          if(isset($order->mobile_no) && $order->mobile_no != "" && strlen($order->mobile_no)==10)
          {
            $mobile_no = $order->mobile_no;
            
            $message='Dear '.$order->customer_name.', Thank you for Shopping at Dadreeios.com! Your Order No. '.$order->orders_counter_id.' has been confirmed and is being processed for shipping. Dadreeios Team G.R. Parwani Trading Co.';
        ...
        if(($order->shipped_stage == 0 || $order->shipped_stage == null || $order->shipped_stage == "") && isset($request->shipped_stage))
        {
          if(isset($order->mobile_no) && $order->mobile_no != "" && strlen($order->mobile_no)==10)
          {
            $mobile_no = $order->mobile_no;
            ...
            // $message='Dear '.$order->customer_name.', Your Order Number '.$order->orders_counter_id.' from Dadreeios.com has shipped via Delhivery. Your Order Tracking Number is '.$order->package_waybill.'\nDadreeios Team,\nParwani Trading Co.';
            $message_url = urlencode($message);
