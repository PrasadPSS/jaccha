...
use App\Models\backend\RelatedVariantProducts;
...
class ProductvariantsController extends Controller
{
    ...
    public function edit($id)
    {
        ...
        $product_images = ProductVariantImages::where('product_variant_id', $products->product_variant_id)->where('color_id', $products->color_id)->get();
        // dd($product_images);
        ...
        $related_product = ProductVariants::where('category_id', $products->category_id)->get();
        $related_product = collect($related_product)->mapWithKeys(function ($item, $key) {
            return [$item['product_variant_id'] => $item['product_title']];
        });
        // dd($related_product);

        $related_product_list_id = RelatedVariantProducts::where('product_variant_id', $products->product_variant_id)->get();
        ...
        // dd($related_product_list);
        return view('backend.productvariants.edit', compact('products', 'categories', 'sub_categories', 'sub_sub_categories', 'countries', 'sellers', 'manufacturers', 'packers', 'importers', 'product_images', 'filter_list', 'filter_values', 'color_list', 'size_list', 'product_variants', 'related_product', 'related_product_list'));
    }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('product_images')) {
            ...
            foreach ($images as $image) {
                ...
                $destinationPath2 = public_path('/backend-assets/uploads/product_variant_imagesxl' . $products->product_variant_id);
                ...
                $product_images->image_name = $id . '/' . $name;
                $product_images->product_id = $products->product_id;
        ...
        if (isset($request->related_product_list_id)) {
            foreach ($request->related_product_list_id as $value) {
                $related_products = new RelatedVariantProducts();
                $related_products->product_variant_id = $products->product_variant_id;
    ...
    public function destroy_images(Request $request)
    {
        ...
        // dd($image_ids);
        $product_images = ProductVariantImages::whereIn('product_image_id', $image_ids)->get();
        // dd($product_images);
