        ...
        if ($backendmenu->permissions)
        {
          ...
          foreach ($base_permissions as $permission)
          {
            if (in_array($permission->base_permission_id,$request->permissions))
            {
              ...
              $permission_name = $permission->base_permission_name.' '.$backendmenu->menu_name;
        ...
        if ($request->permissions)
        {
          ...
          foreach ($base_permissions as $permission)
          {
            if (in_array($permission->base_permission_id,$request->permissions))
            {
              ...
              $menu_permissions = Permission::where('name',$permission_name)->first(); //Match input //permission to db record //->where('menu_id',$request->menu_id)->where('submenu_id',$request->submenu_id)
              dd($permission_name);
              if ($menu_permissions)
              {
