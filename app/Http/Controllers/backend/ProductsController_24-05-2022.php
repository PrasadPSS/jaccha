  ...
  public function __construct()
  {
    ...
    public function create()
    {
      ...
      return view('backend.products.create', compact('categories','sub_categories','sub_sub_categories',
        ...
        'size_chart_list','related_product','frequently_bought',
        'product_gsts','hsncodes','materials','disclaimer'
      ));
    }
...
//      dd($products);
      if($products->save())
      {
          ...
          if ($request->hasFile('product_images'))
        {
          ...
          $destinationPath2 = public_path('/backend-assets/uploads/product_imagesxl');
          if (!file_exists($destinationPath2))
          {
            mkdir($destinationPath2,0777, true);
          }
          $destinationPath_original = public_path('/backend-assets/uploads/product_images_original');
    ...
    public function edit($id)
    {
        ...
        // dd($product_images);
        $colors = Colors::pluck('color_name','color_code')->all();
        $filters = Filters::with('filtervalues')->where('visibility',1)->where('product_page_visibility',1)->where('category_id',$products->category_id)->whereNotIn('filter_type', ['color','size'])->orderBy('category_id')->get();
        foreach ($filters as $filter)
        {
          $filter_name = $filter->filter_name.'->';
          foreach ($filter->filtervalues as $filter_value)
        ...
        return view('backend.products.edit', compact('products','categories','sub_categories','sub_sub_categories',
          ...
          'brand_list','related_product','related_product_list','frequently_bought','frequently_bought_together__list',
          'product_gsts','hsncodes','materials',
        ));
    }
    ...
    public function getfilters(Request $request)
    {
      ...
      $filters = Filters::with('filtervalues')->where('visibility',1)->where('product_page_visibility',1)->where('category_id',$category_id)->whereNotIn('filter_type', ['color','size'])->orderBy('category_id')->get();
