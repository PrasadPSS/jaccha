...
class UsersController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate(
            ...
            [
                ...
                'honey_pot_validation' => ['nullable', 'max:0'], // Honeypot validation
    ...
    public function loginstore(Request $request)
    {
        ...
        if (auth()->attempt(request(['email', 'password'])) == false) {
            ...
            $test = User::where('email', $request->email)->increment('pass_fail_count', 1);
            // dd($test);
            $user_data = User::where('email', $request->email)->get()->toArray();
            $curr_datetime = date('Y-m-d H:i:s');
            // dd($curr_datetime);
            if (!empty($user_data) && $user_data[0]['pass_fail_count'] >= 3) {
                User::where('email', $request->email)->update(['block_status' => 1, 'blocked_datetime' => $curr_datetime]);
            }
    ...
    public function loginpassword(Request $request)
    {
        ...
        if ($current_user) {
            ...
            $curr_datetime = date('Y-m-d H:i:s');
            $time = strtotime($user_data[0]['blocked_datetime']);
            $next_datetime = $time + 7200;//2 hours
            $next_datetime = date('Y-m-d H:i:s', $next_datetime);


            // dd($next_datetime);

            if ($user_data[0]['block_status'] == 1) {
                if ($curr_datetime >= $next_datetime) {
                    $update_id = User::where('email', $email)->update(['block_status' => 0, 'pass_fail_count' => 0]);
                    return view('frontend.users.login_password', compact('email'));
                }
                return back()->withErrors([
                    'message' => 'Your account has been locked due to more than 3 invalid password attempts. Please try again after 2-Hours'
                ]);
            } else if ($current_user->account_status == 0) {
    ...
    public function verifyotp(Request $request)
    {
        ...
        if ($user) {
            ...
            if (($otpDate >= $otpDateBegin) && ($otpDate <= $otpDateEnd)) {
                $user->verified_mobile_no = 1;
                $user->update();
                //for traffic source after verification added by mahesh on 07-02-2023
                $user_id = $user->id;
                $email = $user->email;
                $uid = ['user_id' => $user_id];
                //by ip
                ...
                if (session()->has('cart')) {
                    ...
                    return redirect('/movetocart')->with('success', 'Mobile Number Verified Successfully');
                }
                return redirect()->to('/')->with('success', 'Mobile Number Verified Successfully');
            } else {
                return redirect()->back()->withErrors(['OTP Expired!']);
            }
            // return view('frontend.users.change_password',compact('user'));
