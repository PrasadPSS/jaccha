...
class CartController extends Controller
{
  public function __construct()
  {
    ...
    $this->middleware(
      'auth',
      ['except' => [
        'addtocart', 'payment', 'paymentsuccess',
  ...
  public function index()
  {
    ...
    if (!auth()->user()) {
      ...
      if ($shipping_address) {
        ...
        $company = Company::first();
        $product_wt = get_cart_product_weight();
        $shipping_charge = get_shipping_charges('S', $product_wt, $company->pincode, 'Delivered', '400065');
        $shipping_charge = json_decode($shipping_charge, true);
        $shipping_charges = isset($shipping_charge[0]) ? $shipping_charge[0] : [];
        $shipping_amount = isset($shipping_charge[0]['total_amount']) ? $shipping_charge[0]['total_amount'] : 0;
        //   dd($shipping_charge);
  ...
  public function checkout()
  {
    ...
    $fake_orders = FakeOrderManagement::where(['user_id' => $user_id, 'status' => 'active'])->where('count', '!=', 0)->get()->toArray();

    $order_date = strtok($fake_orders[0]['order_date'], ' ');
    ...
    if ($cod_management) {
      if (($todays_date == $order_date) && ($fake_orders[0]['count'] > $orders_data) && ($fake_orders[0]['status'] == 'active')) {
        ...
        $cod_message = 'You Have Used Your Daily Cash On Delivery Limit (' . $orders_data . ' Times)';
      }
    }
    ...
    if ($cod_response == 'Y') {
      if ($cod_management->cod_purchase_min_limit <= $cart_total_with_discount && $cod_management->cod_purchase_max_limit >= $cart_total_with_discount && ($todays_date == $order_date) && ($fake_orders[0]['count'] > $orders_data) && ($fake_orders[0]['status'] == 'active')) {
        $cod_status = false;
    ...
    if (!($existing_payment)) {
      ...
      if ($payment_info->save()) {
        ...
        if ($cart) {
          ...
          $order->orders_counter_id = "DAD" . $orders_counter_increment_id;
          ...
          if ($order->save()) {
            ...
            $current_order->confirmed_stage = 0;
          ...
          if (isset($current_order) && $current_order->confirmed_stage == 1) {
  ...
  public function Sendneworder($invoicemodel, $payment_mode, $name)
  {
    ...
    try {
      ...
      $message = "
          ...
          <body><div style='margin:auto;'><a href='http://holyhanspariwar.com/'><img src='http://parasightdemo.com/dadreeios/frontend-assets/images/logoparwani.png' width='120'></a></div>
    ...
    if ($coupon_data) {
      ...
      if (($paymentDate >= $contractDateBegin) && ($paymentDate <= $contractDateEnd)) {
        ...
        if ($total_mrp_price >= $coupon_data->coupon_purchase_limit) {
          ...
          return ['error', 'Your net order amount needs to be above ₹ ' . $coupon_data->coupon_purchase_limit . ' to avail this offer!'];
        }
      } else {
  ...
  public function goguestcart()
  {
    ...
    $coupon_data = Coupons::get();
    $shipping_amount = 0;
    if (isset($_COOKIE['cookie_pincode'])) {
      $company = Company::first();
      $product_wt = get_cart_product_weight();
      $shipping_charge = get_shipping_charges('S', $product_wt, $company->pincode, 'Delivered', $_COOKIE['cookie_pincode']);
      $shipping_charge = json_decode($shipping_charge, true);
      $shipping_charges = isset($shipping_charge[0]) ? $shipping_charge[0] : [];
      $shipping_amount = isset($shipping_charge[0]['total_amount']) ? $shipping_charge[0]['total_amount'] : 0;
    } else {
