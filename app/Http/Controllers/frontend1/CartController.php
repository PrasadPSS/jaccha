...
class CartController extends Controller
{
    ...
    public function payment(Request $request)
    {
        ...
        } else if($payment_mode == 'phonepe'){

            $merchantid  = "M22UT8APZDW1M";
    ...
    public function paymentstatus(){
        ...
        if(isset($_POST['merchantId']) && isset($_POST['transactionId']) && isset($_POST['amount']) )
        {
            ...
            }else if($responsePayment['success'] == false && $responsePayment['code'] == "PAYMENT_ERROR")
            {
                return redirect()->route('phonepepaymentfailure', ['responsePayment' => $responsePayment]);
            }else
            {
                return redirect()->route('phonepepaymentpending', ['responsePayment' => $responsePayment]);
            }
        }
    }
    ...
    public function phonepepaymentfailure(){
        $response = $_REQUEST['responsePayment'];
        if (isset(auth()->user()->id)) {
            $user_id = auth()->user()->id;
            $user_session = auth()->user();
        }
        ...
        $txnid = $response['data']['merchantTransactionId'];
        if ($user_session) :
            $missing_payments = MissingPayments::where('user_id', $user_session->id)->Where('transaction_id', $txnid)->first();
        endif;
        if ($missing_payments) :
    ...
    public function phonepepaymentpending(){
        $response = $_GET['responsePayment'];
        // dd($response);
        ...
        if ($missing_payments) :
            $missing_payments->payu_response = 'P';
            $missing_payments->status = "Pending";
            $missing_payments->save();
        endif;
        return view('frontend.cart.payment.pending');
    }
