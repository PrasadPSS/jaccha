...
class AddressesController extends Controller
{
  ...
  public function editaddress($id,$page)
  {
    $user_id = auth()->user()->id;
    $shipping_address = ShippingAddresses::where('user_id',$user_id)->where('shipping_address_id',$id)->first();
    return view('frontend.addresses.edit_address',compact('shipping_address','page'));
  }
  ...
  public function updateaddress(Request $request)
  {
    ...
    if ($update_shipping_address->save())
    {
      if($page == "checkout")
      {
        return redirect()->to('/cart/checkout')->with('success','Address Updated Successfully !');
      }
      else
      {
