...
class ProductController extends Controller
{
    ...
    public function productlist(Request $request, $category_slug = null, $sub_category_slug = null, $sub_sub_category_slug = null, $sort_order = null, $search_query = null)
    {
        ...
        if (isset($request->q) && $request->q != '') {
            ...
            $search .= $search_query;
            if (!isset($products)) {
                $products = Products::select('products.*', 'product_variants.first_variant_id', 'product_variants.color_id as variant_color_id')
        ...
        if (isset($variant_slug)) {
            ...
            $product = ProductVariants::where('category_slug', $category_slug)
                ...
                ->with([
                    ...
                    'importer', 'manufacturer', 'size', 'brands', 'variant_combined_size'
                ])
                ->first();
            // dd($product);
            if ($product->count() > 0) {
                ...
                // dd($size_chart_id);
                $size_chart = SizeCharts::where('size_chart_id', $size_chart_id)->with('chart_childs')->first();
                // dd($size_chart);
                $size_chart_types = [];
                if (isset($size_chart)) {
