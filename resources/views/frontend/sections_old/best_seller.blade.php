			...
			@php $sliderchcnt = 1; @endphp
				@foreach($homepagefeaturedproducts as $section_child)
					@php
						...
						if($section_child->home_page_featured_product_code == 'best-sellers'){
							if($section_child->home_page_featured_product_type == 'auto'){

								$product_ids = DB::table('orders_product_details')
                 ->select(DB::raw('count(*) as product_id_count,product_id'))
                 ->groupBy('product_id')
										 ...
										 <div class="item">
											 <div class=" card ">
													<div class="container1">
														 <img class="img-fluid img-border-bottom" src="{{ asset('backend-assets/uploads/product_images/') }}/{{ isset($product->product_images[0])?$product->product_images[0]->image_name:'' }}" alt="Img">
