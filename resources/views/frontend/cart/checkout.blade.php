		...
		<div class="col-lg-7 col-md-12 col-sm-12 col-12">
            ...
            <div class="col-md-4 col-sm-4 col-12 align-self-end justify-content-center float-right select-other-adr pr-0 pd-left">
              ...
              <input type="hidden" name="shipping_pincode" id="shipping_pincode" value="{{ $shipping_address->shipping_pincode }}">
                  ...
                  <a href="{{ url('/addresses/editaddress/'.$shipping_address->shipping_address_id.'/checkout') }}" class="btn ord-edit">EDIT</a>
            ...
            @endphp
						<div class="row display-products">
							<div class="col-md-3 col-sm-3 col-4 ">
								<div class="">
									<img src="{{ asset('backend-assets/uploads/'.$product_thumb_location.'/') }}/{{ $product_image }}" class="img-fluid" alt="">
								</div>
							</div>
                    ...
                    <span class="delivery-pro-name">
                      ...
                      @endif
									...
									<p><span class="delivery-pro-type">{{ isset($item->{$join_table}->product_title)?$item->{$join_table}->product_title:0 }}</span></p>
									<p>
                    @if(isset($item->{$join_table}) && $item->{$join_table}->product_discounted_price!=null && $item->{$join_table}->product_discount!=0)
                    ...
                    @elseif(isset($item->{$join_table}) && $item->{$join_table}->product_discounted_price!=null && $item->{$join_table}->product_discount==0)
                      <span class="delivery-pro-price">₹ {{$item->{$join_table}->product_discounted_price}}</span>
                    @else
                      <span class="delivery-pro-original mrp-cut">₹ {{isset($item->{$join_table}->product_price)?$item->{$join_table}->product_price:0}}</span>
                    @endif
                    @if(isset($item->{$join_table}) && $item->{$join_table}->product_discount_type!=null && $item->{$join_table}->product_discount_type=='percent' && $item->{$join_table}->product_discount!=0)
                      <span class="delivery-pro-discount">({{ $item->{$join_table}->product_discount }}% OFF)</span>
                    @elseif(isset($item->{$join_table}) && $item->{$join_table}->product_discount_type!=null && $item->{$join_table}->product_discount_type=='flat' && $item->{$join_table}->product_discount!=0)
                      <span class="delivery-pro-discount">(₹ {{ $item->{$join_table}->product_discount }} OFF)</span>
                    @endif
                  </p>
            ...
            @php
              $product_info .= isset($item->{$join_table}->product_title)?$item->{$join_table}->product_title.",":'';
            @endphp
            @endforeach
					</div>
				</div>
              ...
              @foreach($payment_mode as $mode)
                ...
                @endphp
                  <div class="radio payment_methods col-md-6 reviewcart-payment pl-0">
                    <input name="paymentmode" type="radio" value="{{$mode->payment_mode_code}}" class="payment_mode_cod" id="{{$mode->payment_mode_code}}" {{$mode->default_selected==1?"checked":""}} {{$disable}} required />
                    <label for="{{$mode->payment_mode_code}}" class="radio-label">{{$mode->payment_mode_name}}</label>
                  </div>
