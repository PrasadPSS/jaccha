...
@php
    ...
    // dd($path);
@endphp
@if (isset($product_listing_title) && $product_listing_title != '')
...
@section('content')
    ...
    <div class="sticky-filter top-padding">
        <div class="one-fourth" id="filters">p
            <button id="show-hidden-filter" class="sticky-filter-sort-btn w-100 triggerSidebar" type="button"
                name="button">Filters</button>
        </div>
        <div class="one-fourth" id="sortby">
            <button id="show-hidden-sort" class="sticky-filter-sort-btn w-100 triggerSidebar1" type="button"
                name="button1">Sort By</button>
        </div>
    </div>
    <!-- filter menu for mobile view start -->
        ...
        <div class="row sortyby-section no-gutters sidebar-sortby">
            ...
            <div class="col-lg-10 col-sm-12 col-md-12 col-12 ">
                @if (isset($search) && strlen($search) > 0)
                    @if ($search_flag)
                        <h4 id="page_title_count_search">Search Result for <!-- : --> "{!! $search !!}"<span
                                class="product-count" id="product-count"> ( {{ $product_count }} @if ($product_count > 1)
                                    Products
                                @else
                                    Product
                                @endif)</span></h4>
                    @else
                        <h1 id="page_title_count">{!! $search !!}<span class="product-count" id="product-count"> (
                                {{ $product_count }} @if ($product_count > 1)
                                    Products
                                @else
                                    Product
                                @endif)</span></h1>
            ...
            <div class="col-lg-2 col-sm-12 col-md-12 col-12 sort-filter-section hidden-sort sidebar1">
                ...
                <div class="center" id="sort_order">
                    <select name="sources" id="sources" class="custom-select sources "
                        placeholder="{{ $sort_by_options[$sort_order] }}">
                        @foreach ($sort_by_options as $sort_key => $sort_option)
                            <option value="{{ $sort_key }}" @if ($sort_order == $sort_key) selected @endif>
                                {{ $sort_option }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div class="container-fluidcustom pb-5">
        <div class="row product-filter-section sidebar-filter">
            <!-- filter section start -->
            <div class="col-xl-2 col-lg-3 filter-section hidden-filter sidebar">
                    ...
                    <div class="filter-section-div">
                        <div class="row ">
                            ...
                            <div class="col-lg-6 col-md-6 col-sm-6 col-6">
                                <button class="reset-btn " id="reset-filters" type="button" name="button"><span
                                ...
                                <div class="col-lg-12 col-md-12 col-sm-12 col-12 m-auto">
                                    <div class="style-checkbox">
                                        <label class="style">
                                            ...
                                            <input type="radio" name="filter_gender" value="1"
                                                class="filter_ filter_gender"
                                                @if (isset($_REQUEST['gender']) && $_REQUEST['gender'] == 1) checked @endif>
                                            <span class="checkmark"></span>
                                        </label>
                                        <label class="style">
                                            <span class="gender-bold checkbox-text">Women</span>
                                            <input type="radio" name="filter_gender" value="2"
                                                class="filter_ filter_gender"
                                                @if (isset($_REQUEST['gender']) && $_REQUEST['gender'] == 2) checked @endif>
                                            <span class="checkmark"></span>
                                        </label>
                                    </div>
                                </div>
    ...
    @endif -->
                ...
                <section>
                    <div class="row text-center">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-12 my-auto">
                            <div class="page-bg">
                                <ul class="pagination">
                                    {{ $products->links('frontend.pagination.custom', compact('sort_order', 'filter_check')) }}
                                </ul>
                            </div>
                        </div>
                    </div>
                ...
                <section>
                    ...
                    <div class="row pt-4 ">
                        <div class="col-md-7" id="content_prod">
                            @if (!empty($content_prod->contents))
                                <div id="contents" class="contents_toggle">
                                    {!! $content_prod->contents !!}
                                </div>
                                <section class="mt-2">
                                    <p id="btn" class="view_more_btn"></p>
                                </section>
                            @endif
                        </div>
                        <div class="col-md-5">
                            @if (!empty($product_list))
                                <table class="table table-bordered " style="font-size:12px">
                                    <thead>
                                        <tr>
                                            <td>{!! $search !!}</td>
                                            <td style="width:80px;">Best Price</td>
                                        </tr>
                                    </thead>
                                    ...
                                    <tbody>
                                        ...
                                        @foreach ($product_list as $class)
                                            <tr>
                                                <td style="#c00000 !important"><a
                                                        href="{{ url('dp/') }}/{{ $class->category_slug }}/{{ $class->sub_category_slug }}/{{ $class->sub_sub_category_slug }}/{{ $class->product_slug }}"
                                                        target="_blank"
                                                        style="color:#c00000 !important;">{{ $class->product_title }}</a>
                                                </td>
                                                <td>₹ {{ $class->product_discounted_price }}</td>
                                            </tr>
                                        @endforeach
                            ...
                            @endif
                            @if (!empty($content_prod->content_date))
                                <p style="font-size:12px;">This Data was Last Updated on {!! date('F d, Y', strtotime($content_prod->content_date)) !!}</p>
                            @endif
                        </div>
                        ...
                        <div class="col-lg-6">
                            ...
                            <form action="{{ route('suggestion') }}" method="post">
                                ...
                                <div class="text-suggest pb-4">
                                    <textarea class="suggest-box w-100" name="message" rows="8"
                                        placeholder="Your suggestions are very important for us. Please feel free to tell us what you want?" required></textarea>
                                </div>
                                <button class="suggest-btn" type="submit" name="button"><span
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('mouseup', ".noUi-handle-lower, .noUi-handle-upper", function() {
                ...
                $('input[name="filter_check"]:checked').each(function(index) {
                    ...
                    if (Array.isArray(filter_names[category_name]) && filter_names[category_name]
                        .length > 0) {
                ...
                clearTimeout(typingTimer);
                typingTimer = setTimeout(function() {
                    products(sort_order, category_slug, sub_category_slug, sub_sub_category_slug,
                        filter_check, filter_names, null, null, price_array);
            ...
            $("#search_price_filter").on('click', function() {
                ...
                price_array = [low_price, high_price];
                // console.log(price_array);
                products(sort_order, category_slug, sub_category_slug, sub_sub_category_slug, filter_check,
                    filter_names, null, null, price_array);
            });
            ...
            $(".filter_gender").on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var filter_check = '';
                        var filter_names = '';
                        products(sort_order, category_slug, sub_category_slug,
                            sub_sub_category_slug, filter_check, filter_names,
                            filter_gender, search_query);
                        var low_price = 125;
                        ...
                        if (rangeSlider != undefined) {
                            ...
                            document.getElementById('slider-range-value2').innerHTML = '₹' +
                                high_price;
                            document.getElementById('min_value').value = moneyFormat.from(
                                low_price);
                            document.getElementById('max_value').value = moneyFormat.from(
                                high_price);
                        }
                    },
                    error: function(data) {
            ...
            function products(sort_order, category_slug, sub_category_slug, sub_sub_category_slug, filter_check,
                filter_names, filter_gender = null, search_query = null, price_array = null) {
                console.log(filter_check);
                console.log(price_array);
                // alert('test3');
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('#product-count').text(' (' + data[2] + ' Products)');
                        // slider.reloadSlider();
                        ...
                        if (data[0] == 'success') {
                            ...
                            // $('.pager').href += data[3] + sort_order_clause + filter_check_clause;
                            // console.log(sort_order_clause + " and " + filter_check_clause);
                            // var name = sort_order_clause.trim([0])[0, 5]
                            var path = null;
                            var cnt = 1;
                            // var current_url = $("#paginator_current_page").attr("href");
                            // var currentUrl = window.location.href;
                            // console.log('current_url', current_url);
                            var product_count = data[2];
                            var page_count = Math.ceil(product_count / 40);
                            $('.pager_list').each(function() {
                                var page_no = $(this).data('page_id');

                                if (page_no) {
                                    // console.log($(this));
                                    if (newUrl.indexOf("?") !== -1) {
                                        var the_new_url = newUrl + "&" + "page=" + page_no;
                                    } else {
                                        var the_new_url = newUrl + "?" + "page=" + page_no;
                                    }

                                    $(this).attr("href", the_new_url);
                                }
                                // path = this.href += filter_check_clause;
                                // console.log('cnt', cnt++);
                                // console.log('href', this.href);
                                if ($(this).attr('data-page_id') > page_count) {
                                    // console.log($(this));
                            ...
                            // $('.pager_list').hide().filter(function() {
                            //     return page_count;
                            // }).show()

                            // const path_url = path;
                            // const path_data = path_url.split("?");
                            // const path_final = path_data[0] + '?' + path_data[2] + '&' + path_data[1];

                            // $('.pager_list').on('change', function(event) {
                            //     event.preventDefault();
                            //     // this.href.replace(path_final);
                            //     $(this).attr("href", path_final);
                            // })
                            // const path_url = $('.pager_list').attr("href", filter_check_clause);
                            // const path_subdata = path_data[5].split("?");
                            // console.log(path_data);
                            // console.log(path_final);


                            if (filter_check_clause != "" && filter_check_clause != null &&
                                filter_gender != "" && filter_gender != null) {
                                newUrl += '&gender=' + filter_gender;
                            } else if ((filter_check_clause == "" || filter_check_clause == null) &&
                                filter_gender != "" && filter_gender != null) {
                                newUrl += '?gender=' + filter_gender;
                            }
        ...
        $(document).ready(function() {
            ...
            txt = document.getElementById("contents");
            ...
            $('#btn').click(function() {
                if (btn.textContent == 'View More') {
                    $("#contents").removeClass("contents_toggle");
                    btn.innerHTML = 'View Less';
                } else {
                    // alert('less');
                    $("#contents").addClass("contents_toggle");
                    // txt.classList.add = 'contents_toggle';
                    btn.innerHTML = 'View More';
                }
            });
