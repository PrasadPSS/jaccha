...
<!-- <div class="container">
<img class = "cloudzoom" src = "{{ asset('frontend-assets/images/small/image1.jpg') }}"
                          ...
                          @if(isset($product->product_images) && count($product->product_images)>0)
                           @foreach($product->product_images->take(6) as $product_image)
                             <div class="carousel-item {{ ($slidcount==0)?'active':'' }}">
                                <div class="wrapper">
                                   <div class="block">
                                   <img class = "cloudzoom" src = "{{ asset('backend-assets/uploads/product_images_original/') }}/{{ $product_image->image_name }}"
                          ...
                          @if(isset($product->product_images) && count($product->product_images)>0)
                           @foreach($product->product_images->take(6) as $product_image)
                              ...
                              <li data-target="#carousel-thumb " class="{{ ($thumbcount==0 || $thumbcount==3)?'image-mb':'' }} {{ ($thumbcount==2 || $thumbcount==5)?'image-mt':'' }} img-li {{ ($thumbcount==0)?'active':'' }}" data-slide-to="{{ $thumbcount }}">
                                <img class="img-fluid {{ ($thumbcount==0)?'active1':'' }} img-hover-shadow lazy" src="{{ asset('backend-assets/uploads/product_images_original/') }}/{{ $product_image->image_name }}" >
                              </li>
                            ...
                            <input quickbeam="color" id="color-{{ $product->color_id }}" type="radio" name="color_id" value="{{ $product->color_id }}" class="color_checked" checked required  />
                             <label for="color-{{ $product->color_id }}" style="border-color: #000;">
                                @if(isset($product->color) && $product->color->color_type == 'multi')
                                    <img height="20" width="20" style="margin: 0px 0px 0px 0px !important;" src="{{ asset('backend-assets/uploads/color_code/') }}/{{ isset($color_list[$product->color_id])?$color_list[$product->color_id]:'' }}" alt="Color">
                                  @else 
                                                ...
                                                <div class="tab-pane container text-center fade" id="sizechart-img">
                                                  @if(isset($size_chart->chart_images) && count($size_chart->chart_images)>0)
                                                    @foreach($size_chart->chart_images as $size_chart_image)
                                                      <img class="img-fluid mt-3  mb-3 lazy" src="{{ asset('backend-assets/uploads/size_chart_images/') }}/{{ $size_chart_image->image_name }}" alt="">
                                                    @endforeach
                                                  @endif
                                                </div>
                                             </div>
                                   ...
                                   <div class="row content-ratinng">
                                      <div class="col-lg-2 col-md-2 col-sm-5 col-5">
                                         <div class="produtct-rating-img">
                                           <img class="img-fluid lazy" src="{{ asset('backend-assets/uploads/product_thumbs/'.$product->product_thumb) }}" alt="Banner">
                                         </div>
                                      </div>
