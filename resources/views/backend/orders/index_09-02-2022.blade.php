                                    ...
                                    <tr>
                                      <td>{{ $srno }}</td>
                                        ...
                                        <td>
                                            ...
                                            <a href="{{url('admin/orders/invoice/'.$order->order_id)}}" class="btn btn-primary btn-sm mt-1" >Download Invoice</a>
                                            @if($order->package_order_status==0)
                                              <a href="{{url('admin/orders/createpackageorder/'.$order->order_id)}}" class="btn btn-primary btn-sm mt-1" >Create Package Order</a>
                                            @endif
                                            @if($order->package_order_status==1)
                                              <a href="{{url('admin/orders/generatepacakgeslips/'.$order->order_id)}}" class="btn btn-primary btn-sm mt-1" >Generate Package Slips</a>
                                            @endif
                                            @if($order->preparing_order_stage==1)
                                              <a href="{{url('admin/orders/generateproductlabels/'.$order->order_id)}}" class="btn btn-primary btn-sm mt-1" >Generate Product Labels</a>
                                            @endif
                                            <a href="{{url('admin/orders/details/'.$order->order_id)}}" class="btn btn-primary btn-sm mt-1" >View Details</a>
                                        </td>
                                    </tr>
                                    @php $srno++; @endphp
                                    @endforeach
